...
export const credentialsRouter = createTRPCRouter({ 
  create: protectedProcedure
    .input( 
      z.object({ 
        ...
        value: z.string().min(1, "Value (valid key) is required")
      })
    )
    .mutation(({ ctx, input }) => { 
    ...
    return prisma.credential.create({ 
      data: { 
        ...
        type,
        value,  // To do: recommend encrypting in production (Amazon Secrets Manager)
      },
    });
  }),
  ...
  getOne: protectedProcedure
    .input(z.object({ id: z.string() }))
    .query(({ ctx, input }) => { 
      return prisma.credential.findUniqueOrThrow({  // throws error if Credential is not found (null)
